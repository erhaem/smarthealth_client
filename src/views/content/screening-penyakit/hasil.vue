<template>
  <div class="container py-5">
    <h2 class="text-center mb-4 fw-semibold">HASIL SKRINING</h2>
      ...
      <table class="table table-hover border">
        ...
        <tbody>
          <tr v-for="data in sortedResults" :key="data.id">
            ...
            <td>
              <button class="btn btn-primary" @click="viewDetails(data.id)">Lihat Detail</button>
            </td>
          </tr>
        </tbody>
      </table>
...
function konversiArrayPHPkeJS(arrayPHP) {
  ...
  return ['Tidak ada penyakit yang diskrining', 0]
}
...
export default {
  ...
  methods: {
    ...mapActions('diagnosis', ['detail_skrining']),
    async fetchData() {
      try {
    ...
    viewDetails(id) {
      this.detail_skrining(id)
      this.$router.push({ name: 'detail_skrining', params: { id } })
    },
    getDiagnosedDisease(data) {
      ...
      return 'Tidak ada penyakit yang diskrining'
    },
