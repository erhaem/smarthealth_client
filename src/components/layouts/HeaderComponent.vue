      ...
      <div class="collapse navbar-collapse justify-content-end" id="navbarNavDropdown">
        <ul class="navbar-nav">
          ...
          <li
            ...
            v-if="isAuthenticated && user.data.getRole.idRole === 'RO-2003064'"
          >
            <a
            ...
            <DropdownItem>
              <template #item>
                <router-link :to="{ name: 'Riwayat Konsumen' }" class="dropdown-item"
                  >Kunjungan Medis</router-link
                >
          ...
          <li v-if="!isAuthenticated" class="nav-item dropdown">
            ...
            <DropdownItem>
              <template #item>
                <router-link :to="{ name: 'RegisterKonsumen' }" class="dropdown-item"
                  >Member</router-link
                >
                <router-link :to="{ name: 'RegisterPerawat' }" class="dropdown-item" href="#"
                  >Tenaga Medis</router-link
                >
              </template>
            </DropdownItem>
          </li>
...
export default {
  ...
  methods: {
    ...
    getTotalKeranjang() {
      let type = 'getData'
      let url = ['keranjang/total/by_konsumen', {}]
      this.$store
        .dispatch(type, url)
        .then((result) => {
